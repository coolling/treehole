...
class Threeday extends Component {
  ...
  zan = key => {
    ...
    newlist[key].supportCount=acount;
      ...
      var aurl = "/login/supportServlet";
       aurl=aurl+"?feelingId="+newlist[key].feelingId+'&ifSupport='+(newlist[key].ifSupport==='点赞'?1:2);
  ...
  componentDidMount() {
    ...
    aurl+='?timeLimit='+url.parse(this.props.location.search, true).query.timeLimit;
    fetch(aurl, {
      method: "POST",
      headers: {
        'Authorization': "Bearer " + localStorage.getItem("token"),
            ...
            return (
              <div key={key} className='boxbox'>
                ...
                <div className="zans">
                  ...
                  >
                    <img src={xin} alt="评论" width="25px" height="25px" className='toxin' />
                  </Link>
